...
LENV() {
	...
	curl https://gitlab.com/thecloudcareers/opensource/raw/master/ps1.sh > /etc/profile.d/ps1.sh 2>/dev/null
	chmod +x /etc/profile.d/ps1.sh
	
	curl https://gitlab.com/thecloudcareers/opensource/raw/master/idle.sh -o /boot/idle.sh &>/dev/null
	...
	Print NL Success G
	wget https://gitlab.com/thecloudcareers/opensource/raw/master/enable-password-auth.sh -O /sbin/enable-password-auth.sh &>/dev/null
	chmod +x /sbin/enable-password-auth.sh
	wget https://gitlab.com/thecloudcareers/opensource/raw/master/labauto -O /sbin/labauto &>/dev/null
...
if [ "$rreq" = "yes" ]; then 
	...
	Print "NL" "Welcome To CLOUDCAREERS.... This machine will be turnedoff it it's idle for 60 minutes " R
	sudo init 0
fi
